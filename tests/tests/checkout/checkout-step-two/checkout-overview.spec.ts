...
test.describe('Checkout Step Two - Overview', () => {
  test('should display product information', async ({ checkoutStepTwoPageReady }) => {
  ...
  test('should calculate subtotal correctly', async ({ checkoutStepTwoPageReady }) => {
    const subtotal = await checkoutStepTwoPageReady.getSubtotal();
    expect(subtotal).toBe(inventoryData.products[0].price);
  });

  test('should calculate tax', async ({ checkoutStepTwoPageReady }) => {
    const subtotal = await checkoutStepTwoPageReady.getSubtotal();
  ...
  test('should cancel and return to inventory', async ({ checkoutStepTwoPageReady, page }) => {
    await checkoutStepTwoPageReady.cancel();

    await expect(page).toHaveURL(/\/inventory\.html/);
  });

  test('should finish order and navigate to complete page', async ({ checkoutStepTwoPageReady, checkoutCompletePage, page }) => {
